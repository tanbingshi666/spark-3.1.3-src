...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    // 2 创建 SparkEnv (创建了很多核心组件)
    // 底层会创建 RpcEnv (NettyRpcEnv)
    ...
    // 启动 SparkUI (基于 Jetty) 并绑定很多 handler
    _ui.foreach(_.bind())
    ...
    // 7 创建并初始化 LocalDiskShuffleDriverComponents
    ...
    // 9 创建 YarnClusterSchedulerBackend, YarnClusterScheduler
    ...
    // 11 向 _heartbeatReceiver 发送一个 TaskSchedulerIsSet 请求
    // 调用其 HeartbeatReceiver.receiveAndReply()
    ...
    // Post init
    // 16 初始化 ApplicationMaster 的 sparkContextPromise
...
object SparkContext extends Logging {
  ...
  private def createTaskScheduler(
    ...
    // 1 根据 master 地址判断执行模式
    master match {
      // local 模式下 也即 setMaster("local")
      case "local" =>
      ...
      // yarn cluster/client 模式下
      case masterUrl =>
        // 创建 YarnClusterManager
        val cm = getClusterManager(masterUrl) match {
          case Some(clusterMgr) => clusterMgr
          case None => throw new SparkException("Could not parse Master URL: '" + master + "'")
        }
        try {
          // 根据 yarn 部署 cluster/client 创建对应的 Scheduler
          // cluster => new YarnClusterScheduler(sc)
          // client  => new YarnScheduler(sc)
          val scheduler = cm.createTaskScheduler(sc, masterUrl)
          // 创建对应的 SchedulerBackend
