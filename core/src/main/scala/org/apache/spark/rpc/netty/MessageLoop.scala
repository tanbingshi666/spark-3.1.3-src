...
private class SharedMessageLoop(
  ...
  override def post(endpointName: String, message: InboxMessage): Unit = {
    ...
    // 往 Inbox 的 messages 添加 InboxMessage
    inbox.post(message)
    // 往 MessageLoop 的 active 添加 Inbox
    setActive(inbox)
  }
  ...
  def register(name: String, endpoint: RpcEndpoint): Unit = {
    // 1 创建 Inbox (注意往自己的 message 添加了一个 OnStart 事件)
    val inbox = new Inbox(name, endpoint)
    // 2 缓存 Inbox
    endpoints.put(name, inbox)
    // Mark active to handle the OnStart message.
    // 3 处理 OnStart 事件
    // 注意 MessageLoop 内置了一个线程 基于线程池的方式调用
    // 最终调用其 receiveLoop()
    setActive(inbox) // 往自己的 active 添加 inbox
  }
}
