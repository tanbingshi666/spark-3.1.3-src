...
private[netty] class NettyRpcEnv(
  ...
  // 2 创建 Dispatcher 维护 Rpc Endpoint Ref
  private val dispatcher: Dispatcher = new Dispatcher(this, numUsableCores)

  // 3 创建 NettyStreamManager
  private val streamManager = new NettyStreamManager(this)

  // 4 创建 TransportContext 主要创建 Server
  private val transportContext = new TransportContext(transportConf,
    // 5 创建 NettyRpcHandler 处理 rpc 请求
  ...
  // 6 创建 TransportClientFactory 主要创建 Client
  ...
  def startServer(bindAddress: String, port: Int): Unit = {
    ...
    // 1 创建 TransportServer 并启动
    server = transportContext.createServer(bindAddress, port, bootstraps)

    // 2 往 Dispatcher 注册 TransportServer RpcEndpoint
  ...
  def asyncSetupEndpointRefByURI(uri: String): Future[RpcEndpointRef] = {
    // 1 封装 uri 为 RpcEndpointAddress
    val addr = RpcEndpointAddress(uri)

    // 2 创建 NettyRpcEndpointRef
    val endpointRef = new NettyRpcEndpointRef(conf, addr, this)

    // 3 创建 NettyRpcEndpointRef
    val verifier = new NettyRpcEndpointRef(
      conf, RpcEndpointAddress(addr.rpcAddress, RpcEndpointVerifier.NAME), this)

    // 4 发送一个 CheckExistence RPC 请求服务端 判断其是否存在
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    ...
    if (!config.clientMode) {
      val startNettyRpcEnv: Int => (NettyRpcEnv, Int) = { actualPort =>
        // 3 启动 Rpc Server 也即完成 Netty 的配置 比如 BossGroup、WorkGroup 的创建等以及启动 Netty Server
