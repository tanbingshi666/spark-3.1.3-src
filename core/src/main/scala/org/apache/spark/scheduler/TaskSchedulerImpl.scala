...
private[spark] class TaskSchedulerImpl(
  ...
  override def start(): Unit = {
    // 启动 YarnClusterSchedulerBackend
    backend.start()
  ...
  override def submitTasks(taskSet: TaskSet): Unit = {
    // 1 获取 stage 对应的 Tasks
    val tasks = taskSet.tasks
    logInfo("Adding task set " + taskSet.id + " with " + tasks.length + " tasks "
      + "resource profile " + taskSet.resourceProfileId)
    this.synchronized {
      // 2 创建 TaskSetManager 管理 TaskSet
      val manager = createTaskSetManager(taskSet, maxTaskFailures)
      val stage = taskSet.stageId

      // 3 初始化 stage 对应的 TaskSetManager 容器
      ...
      // 3 设置 TaskSet 对应的 TaskSetManager
      stageTaskSets(taskSet.stageAttemptId) = manager

      // 4 将 TaskSetManager 加入到调度器 (默认 FIFO) 等待调度
    ...
    // 5 执行调度 TaskSet
    // 调用 CoarseGrainedSchedulerBackend.reviveOffers()
    // 最终调用 DriverEndpoint.makeOffers()
