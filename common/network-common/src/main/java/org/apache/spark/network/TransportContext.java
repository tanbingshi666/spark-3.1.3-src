...
public class TransportContext implements Closeable {
    ...
    public TransportChannelHandler initializePipeline(
        ...
        try {
            // 1 创建 TransportChannelHandler 也即 request channel handler
    ...
    private TransportChannelHandler createChannelHandler(Channel channel, RpcHandler rpcHandler) {
        // 1 创建 TransportResponseHandler 响应消息给客户端
        ...
        // 3 创建 TransportRequestHandler 接受客户端消息并处理
        TransportRequestHandler requestHandler = new TransportRequestHandler(channel, client,
                rpcHandler, conf.maxChunksBeingTransferred(), chunkFetchRequestHandler);

        // 4 创建 TransportChannelHandler 也即继承了 Netty 的 SimpleChannelInboundHandler
        // 接受/响应客户端消息核心在于 TransportChannelHandler.channelRead0()
